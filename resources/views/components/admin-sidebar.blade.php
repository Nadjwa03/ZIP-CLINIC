<div class="h-full w-full flex flex-col py-4">
  ...
  <nav class="flex-1 flex flex-col gap-y-2 px-3 py-3 mt-3 overflow-y-auto">
    ...
    <!-- Doctors -->
    <a href="{{ route('admin.doctors.index') }}" class="w-full flex gap-x-2 p-3 group hover:bg-emerald-50 rounded-md {{ request()->routeIs('admin.doctor.*') ? 'bg-emerald-50' : '' }}">
      <svg xmlns="http://www.w3.org/2000/svg" class="fill-current {{ request()->routeIs('admin.doctor.*') ? 'text-emerald-600' : 'text-neutral-700' }} group-hover:text-emerald-600 w-6 h-6" viewBox="0 -960 960 960">
      ...
      <span class="{{ request()->routeIs('admin.doctor.*') ? 'text-emerald-600' : 'text-neutral-700' }} group-hover:text-emerald-600">Dokter</span>
    </a>
