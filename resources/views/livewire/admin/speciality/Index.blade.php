    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-blue-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600">Total Spesialisasi</p>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Spesialisasi</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {{ $specialities->firstItem() + $index }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-medium text-gray-900">{{ $speciality->speciality_name }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-600">
                            {{ $speciality->description ? Str::limit($speciality->description, 50) : '-' }}
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">
                            {{ $speciality->doctors()->count() }} dokter
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div class="flex items-center justify-center gap-2">
                            <!-- Toggle Status -->
                            <button 
                                wire:click="toggleStatus({{ $speciality->speciality_id }})"
                                wire:confirm="Ubah status spesialisasi {{ $speciality->speciality_name }}?"
                                class="px-3 py-1 {{ $speciality->is_active ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }} rounded transition text-xs">
                                {{ $speciality->is_active ? '⏸ Nonaktifkan' : '▶ Aktifkan' }}
                            </button>
                            ...
                            <!-- Delete -->
                            <button 
                                wire:click="deleteSpeciality({{ $speciality->speciality_id }})"
                                wire:confirm="Hapus spesialisasi {{ $speciality->speciality_name }}? Data tidak dapat dikembalikan!"
